...
# Direct-KMS only adds metadata behavior. It deliberately does not touch the
# existing PRAGMA registration block produced by prepare_extension_java_compat_final.py.
...
# The external KMS ID only replaces footer key_metadata on that explicit path.
...
print('Direct-KMS metadata overlay applied without changing PRAGMA API:', DST)
